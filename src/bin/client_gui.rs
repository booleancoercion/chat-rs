...
use tokio::sync::mpsc;
use iced::{Element, Application, executor, Command, Settings, Text, Column, button, Button,
           text_input, TextInput, Length, HorizontalAlignment, Container, Row,
           Align, Color, scrollable, Scrollable, Subscription};
...
impl Application for ChatClient {
    ...
    fn title(&self) -> String {
        format!("chat-rs{}", if let ChatClient::Ready { peer_addr, .. } = self {
    ...
    fn update(&mut self, message: Self::Message) -> Command<Self::Message> {
        ...
        match self {
                        ...
                        *self = ChatClient::Connecting(None);
                        return Command::perform(async move {
                            ...
                            match stream.receive_msg(&mut buffer).await {
                                ...
                                Err(e) => bail!("Error connecting to server: {}", e.to_string())
                            }
            ...
            ChatClient::Ready { messages, writer_channel, state, .. } => {
                match message {
                    AppMessage::ChatMsg(msg) => {
                        ...
                        if !state.scroll.is_scroller_grabbed() {
                            ...
                            state.scroll = unsafe {
                                let mut tmp = std::mem::transmute::<_, (Option<f32>, f32)>(state.scroll);
                                tmp.1 = 999999.0;
    ...
    fn subscription(&self) -> Subscription<Self::Message> {
        match self {
            ChatClient::Ready {msg_mpsc: mpsc, ..}
            | ChatClient::Connecting(Some((_, mpsc))) => {
...
impl AppMessage {
    pub fn or_error<T>(g: impl (Fn(T) -> AppMessage) + 'static + Send)
...
mod style {
    use iced::{button, Background, Vector, container, Color};
